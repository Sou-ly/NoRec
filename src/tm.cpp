...
#define MAX_SEGMENTS 65536 // 2^(64 - 48) = 2^16 as we allocate no more than 2^48
...
shared_t tm_create(size_t size, size_t align) noexcept
{
    ...
    region->segments = std::vector<std::vector<word_t>>(MAX_SEGMENTS);
...
bool tm_end(shared_t shared, tx_t tx) noexcept
{
    ...
    reg->free_set.insert(trx->free_set.begin(), trx->free_set.end());
    if (unlikely(reg->running.load() == 1)) // only this transaction
    {
        std::cout << "freeing memory\n";
        for (auto &addr : reg->free_set)
        {
            reg->segments[addr].clear();
        }
        reg->free_set.clear();
    }
...
bool tm_read(shared_t shared, tx_t tx, void const *source, size_t size, void *target) noexcept
{
    ...
    {
        word_t *target_addr = reinterpret_cast<word_t *>((uintptr_t)target + i);
        uintptr_t source_addr = (uintptr_t)source + i;
...
bool tm_write(shared_t shared, tx_t tx, void const *source, size_t size, void *target) noexcept
{
    ...
    for (size_t i = 0; i < size; i+=reg->align)
    {
        uintptr_t target_addr = (uintptr_t)target + i;
        const word_t *source_addr = reinterpret_cast<const word_t *>((uintptr_t)source + i);
...
{
    transaction* trx = reinterpret_cast<transaction*>(tx);
    trx->free_set.insert(((uintptr_t)target >> 48));
    return true;
}